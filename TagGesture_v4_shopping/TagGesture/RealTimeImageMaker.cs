...
namespace TagGesture
{
    class RealTimeImageMaker
    {
        ...
        public Image GetCurrentCurve()
        {
            ...
            g.DrawPath(new Pen(Color.Pink), path3);
        ...
        public void updatePointY(DataRow row)
        {
            ...
            // 更新数据点
                ...
                //曲线上每个点的纵坐标随机生成，但保证在显示区域之内 
                float tempY;
                float.TryParse(tagInfo[1].ToString(), out tempY);
                tempPointF.Y = tempY * 5 + height / 2;

                if (tagInfo[0].ToString() == "EEEE")  //颜色Green
                { 
                    //在最后再添加一个新坐标点  
                    pointListF[width - 1] = tempPointF;
                }

                if (tagInfo[0].ToString() == "FFFF")  //颜色Red
                {
                    pointListF2[width - 1] = tempPointF;
                }

                if (tagInfo[0].ToString() == "E099") //颜色Pink
                {
                    pointListF3[width - 1] = tempPointF;
                }
